...
const Billing = () => {
    ...
    if (freePlanOrders.length > 0) {
      ...
      freePlanOrders.forEach((order) => {
        notifications.show({
          ...
          message: `Your ${
            order.plan?.name || "Free"
          } plan has been activated! No payment required.`,
          color: "green",
          icon: <IconCheck />
        })
      })
  ...
  const totalCredits = currentSubscription?.planId?.credits || 0
  ...
  const totalDays =
    (creditExpiryDate - lastRechargeDate) / (1000 * 60 * 60 * 24)
  ...
  const handleCreateCheckout = async (order) => {
    ...
    if (!order.orderId) {
      notifications.show({
        title: "Invalid Order",
        message:
          "This order cannot be processed. It may be a free plan that doesn't require payment.",
        color: "yellow",
        icon: <IconInfo />
      })
      return
  ...
  const daysRemaining = Math.ceil(
    (creditExpiryDate - currentDate) / (1000 * 60 * 60 * 24)
  )
  ...
  return (
    <>
      <Stack gap="xl">
              ...
              <Box>
                <Text fw={600} size="sm">
                  {isOverdue
                    ...
                    : `Your credits are expiring in ${daysRemaining} day${
                        daysRemaining !== 1 ? "s" : ""
                      }`}
                </Text>
                <Text size="xs" c="dimmed" mt={4}>
                  Purchase new credits or pay any invoice to reset the expiry
                  timer.
                </Text>
              </Box>
        ...
        >
          <Stack>
            {ordersExpiringSoon.filter(
              (order) => order.orderId && order.paymentIntentId
            ).length > 0 && (
              <Alert
                icon={<IconWarning />}
            ...
            {expiredOrders.filter(
              ...
              >
                ...
                expired order(s). These orders have been automatically
                cancelled.
              </Alert>
            )}
              ...
              {pendingOrders
                ...
                .map((order) => (
                  <Alert
                    ...
                    variant="light"
                  >
                    Your {order.plan?.name || "Free"} plan has been activated
                    successfully! No payment required.
                  </Alert>
                ))}
                          ...
                          {isCreditPackageOrder ? (
                            ...
                            >
                              <Text size="xs">
                                ...
                                credits that will be added to your account
                            ...
                            >
                              <Text size="xs">
                                {isUpgrade ? (
                                  ...
                                  <>
                                    <strong>Plan Change:</strong> You'll switch
                                    to {order.planName} plan with{" "}
                                    {orderPlanCredits.toLocaleString()} credits.
                                  </>
                                ) : (
                                  <>
                                    <strong>Purchase Credits:</strong> You'll
                                    receive {orderPlanCredits.toLocaleString()}{" "}
                                    additional credits.
                                  </>
                                )}
                              </Text>
                        ...
                        {usageLoading ? (
                          ...
                          <Tooltip label="Credits used in the selected date range">
                            <Text
                              ...
                              fw={700}
                              c={
                                totalCreditsUsed > totalCredits * 0.8
                                  ? "red"
                                  : "dark"
                              }
                              style={{cursor: "help"}}
